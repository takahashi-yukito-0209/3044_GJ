		...
		fishingChanged |= ImGui::DragFloat(
			...
			0.05f, -100000.0f, 100000.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp
				...
				ImGui::TextDisabled(
					SelectEditorText(
						editorLanguage_,
						"MeshRendererとActiveな非Trigger Box／Sphere Colliderを使用するStatic障害物です。Sphereは最大World軸Scaleを使用します。",
						"Uses MeshRenderer and an active non-trigger Box or Sphere Collider as a static obstacle. Sphere uses the largest World axis scale."
					)
				);
